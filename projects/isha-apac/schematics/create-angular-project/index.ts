import { normalize } from '@angular-devkit/core';
import {
  apply,
  chain,
  externalSchematic,
  MergeStrategy,
  mergeWith,
  move,
  Rule,
  SchematicContext,
  Tree,
  url
} from '@angular-devkit/schematics';
import { addModuleImportToModule } from '@angular/cdk/schematics';
import { BaseSchema } from '../base.schema';
import { addFilesToStyles } from '../helpers/angular-json.helper';
import { ngAddExternal } from '../helpers/schematics-helper';
import { AppModulePath, MaterialModuleImportPath, Packages, SchematicCollection } from '../schematics.constants';
import { NgNewSchema, Style } from './schema';

// @ts-ignore
export function createAngularProject(options: BaseSchema): Rule {
  // @ts-ignore
  return async (host: Tree, context: SchematicContext) => {
    context.logger.log('info', 'Setting up a new Angular Project');
    // return chain([angularProjectSchematicPrivate(options)]);
    const angularOptions: NgNewSchema = {
      ...options,
      name: options.project,
      version: Packages.Angular.version,
      directory: options.project,
      style: Style.Scss,
      inlineStyle: false,
      inlineTemplate: false
    };

    return chain([externalSchematic(Packages.Angular.name, 'ng-new', angularOptions)]);
  };
}

// @ts-ignore
export function angularProjectSchematicPrivate(options: BaseSchema): Rule {
  return (_tree: Tree, _context: SchematicContext) => {
    const angularOptions: NgNewSchema = {
      ...options,
      name: options.project,
      version: Packages.Angular.version,
      directory: options.project,
      style: Style.Scss,
      inlineStyle: false,
      inlineTemplate: false
    };

    return chain([externalSchematic(Packages.Angular.name, 'ng-new', angularOptions)]);
  };
}

// @ts-ignore
function addAngularProject(options: BaseSchema): Rule {
  // @ts-ignore
  return (host: Tree, context: SchematicContext) => {
    const packages = [Packages.AngularMaterial];
    return ngAddExternal(packages, SchematicCollection.MaterialSchematicPrivate, options);
  };
}

// @ts-ignore
function copyResources(): Rule {
  return () => {
    const templateSource = apply(url('./files'), [move(normalize(``))]);
    return mergeWith(templateSource, MergeStrategy.Overwrite);
  };
}

// @ts-ignore
function addStylesToAngularJsonFile(options: BaseSchema): Rule {
  // @ts-ignore
  return (host: Tree, context: SchematicContext) => {
    addFilesToStyles(options, ['src/assets/theme.scss'], host);
    return host;
  };
}

// @ts-ignore
function addImportExportToModule(): Rule {
  return (host: Tree) => {
    addModuleImportToModule(host, AppModulePath, 'MaterialModule', MaterialModuleImportPath);
    return host;
  };
}
